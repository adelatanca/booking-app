    ...
    <>
      ...
      <div className='mt-2 gap-2 grid grid-cols-3 md:grid-cols-3 lg:grid-cols-6'>
        {addedPhotos &&
          ...
          addedPhotos.map((link: string, key: number) => (
            <div className='h-32 flex relative' key={`${key}-link`}>
              <img
                className='rounded-2xl w-full object-cover position-center'
                src={'http://localhost:4000/uploads/' + link}
              />
              <button
                onClick={(ev) => removePhoto({ ev, filename: link })}
              ...
              <button
                onClick={(ev) => setAsMainPhoto({ ev, filename: link })}
